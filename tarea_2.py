...
while True:
    ...
    letr = re.findall(r'[a-z]{1,2}', str, re.I) #metodo que encuentra elementos formados por 1 a 2 letras alfabeticas y crea un lista de strings con los elementos
    npro = int(num[0]) #almacena el primer elemento de la lista num transformado a int. Representa la provincia de una cedula valida
    ...
    if re.fullmatch(r'^\d{1,2}(pi|av|)-\d{1,4}-\d{1,5}$', str, re.I) != None: #regex que coincide con el formato de cedula tipo regular o poblacion indigena o antes de vigencia
        if npro<=13 and npro>0: #check si el numero de provincia coincide con un elemento real
            print("cedula valida") #mensaje de salida
            if not letr:
                print("tipo de cedula: Regular") #
            elif letr[0].lower() =="pi":
                print("tipo de cedula: Poblacion indigena") #
            else:
                print("tipo de cedula: Antes de la vigencia") #
            print("provincia: " + prov[npro-1] + " tomo: " + num[1] + " asiento: " + num[2]) #
            break
        else:
